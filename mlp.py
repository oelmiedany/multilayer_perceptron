...
class MLP:
    ...
    def train(self, inputs, targets, eta, niterations):
        ...
        for n in range(niterations):
            ...
            #derivative determined using sigmoidal function
            sigmoidal = 1/(1+np.exp(-self.beta*self.outputs))#sigmoidal = yk = 1/(1+exp(-beta*outputs))
            deltao = (self.outputs-targets)*(sigmoidal*(1-sigmoidal))#derivative of softmax = yk(1-yk)
            deltao = deltao/ndata

            # compute the derivative of the second hidden layer
            deltah2 = self.hidden2*self.beta*(1-self.hidden2)*(np.dot(deltao,self.weights3.T))#derivative calculated using approach outlined in labs

            # compute the derivative of the first hidden layer
            deltah1 = self.hidden1*self.beta*(1-self.hidden1)*(np.dot(deltah2[:,:-1],self.weights2.T))#derivative calculated using approach outlined in labs
            ...
            #weight increment = learning rate * (the layer's input . derivative of the  layer)
    ...
    def forwardPass(self, inputs):
        ...
        self.hidden1 = 1/(1+np.exp(-self.beta*self.hidden1))#sigmoid function
        self.hidden1 = np.concatenate((self.hidden1,-np.ones((np.shape(inputs)[0],1))),axis=1)#adds bias
        ...
        self.hidden2 = 1/(1+np.exp(-self.beta*self.hidden2))#sigmoid function
        self.hidden2 = np.concatenate((self.hidden2,-np.ones((np.shape(self.hidden1)[0],1))),axis=1)#adds bias

        # output layer
        # compute the forward pass on the output layer with softmax function
        outputs = np.dot(self.hidden2,self.weights3)     
        exp = np.exp(outputs)#exponential
        sum = np.full((np.shape(outputs)[0]), np.sum(exp,axis=1))#sum of exponential
        outputs = (exp.T/sum).T#softmax function = exponential / sum of exponential
